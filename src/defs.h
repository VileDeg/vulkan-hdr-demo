...
#ifdef NDEBUG
    #define TRAP() do{ PREXIT(TEXT_TRAP_MSG); dump_to_log_file(TEXT_TRAP_MSG); EXIT(1); }while(0)
#else
    #ifdef _MSC_VER
...
#define HASSERTMSG(_x, _tru, _msg) do{ \
        if ((_x) != _tru) { \
            PRERR(_msg << ".\n GENERATED abort_log.txt in working directory.\n" << TEXT_ABORT_MSG); \
            dump_to_log_file(TEXT_ABORT_MSG); \
            TRAP(); \
        } \
    } while(0)
